...
const AI_CREDENTIAL_ACCOUNT: &str = "ai-model-api-key";
...
fn ai_credential_entry() -> Result<keyring::Entry, String> {
    keyring::Entry::new(CREDENTIAL_SERVICE, AI_CREDENTIAL_ACCOUNT)
        .map_err(|error| error.to_string())
}
...
#[tauri::command]
pub fn save_ai_credential(api_key: String) -> Result<(), String> {
    let entry = ai_credential_entry()?;
    let api_key = api_key.trim();
    if api_key.is_empty() {
        return delete_ai_credential();
    }
    entry
...
#[tauri::command]
pub fn load_ai_credential() -> Result<Option<String>, String> {
    let entry = ai_credential_entry()?;
