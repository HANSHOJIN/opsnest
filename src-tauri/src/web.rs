...
fn collect_topics(value: &Value, results: &mut Vec<WebSearchResult>) {
    let Some(topics) = value.as_array() else {
        return;
    };
    for topic in topics {
...
pub async fn search_web(request: WebSearchRequest) -> Result<Vec<WebSearchResult>, String> {
    ...
    if query.len() > 500 {
        return Err("搜索内容过长。".to_string());
    }

    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_static("OpsNest/0.1 local agent"),
    );
    ...
    if let Some(topics) = payload.get("RelatedTopics") {
        collect_topics(topics, &mut results);
    }
