...
import { buildNasDetailMetrics, buildRouterOverviewFooter, buildStandardDetailMetrics } from "./features/servers/detail-metrics";
import { ServerDetailActionBar, ServerDetailHeader } from "./features/servers/detail-chrome";
...
function ServerDetailView({ server, text, language, onBack, onOpen, onConnect, onEdit, onManager, onCron }: { server: Server; text: typeof zh; language: Locale; onBack: () => void; onOpen: () => void; onConnect: () => void; onEdit: () => void; onManager: () => void; onCron: () => void }) {
  ...
  return <section className="server-detail-view">
    <ServerDetailHeader server={server} text={text} language={language} eyebrow={zhMode ? "单机详情" : "Server details"} onBack={onBack} />
...
/* Legacy Linux detail implementation retained as a migration reference. */
/*
function ServerDetailViewDynamic({ server, text, language, onBack, onOpen, onConnect, onScan, isScanning, onDiscover, isDiscovering, onEdit, onManager, onCron, onAddCustomService, onDeleteCustomService }: ServerDetailProps) {
  ...
  const detailMetrics = buildStandardDetailMetrics(server, text, zhMode);
  ...
  return <section className="server-detail-view">
    <ServerDetailHeader server={server} text={text} language={language} eyebrow={zhMode ? "单机详情" : "Server details"} onBack={onBack} />
    <ServerDetailActionBar connected={connected} language={language} onOpen={onOpen} onConnect={onConnect} onManager={onManager} onEdit={onEdit} />
    <section className="detail-overview-card"><div className="detail-overview-heading"><div><p className="eyebrow">{zhMode ? "运行概览" : "Overview"}</p><h2>{zhMode ? "这台服务器现在怎么样？" : "How is this server doing?"}</h2><span>{zhMode ? "连接后自动读取系统、资源和已发现服务。" : "System resources and detected services are read after connecting."}</span></div><span className={`latency-badge ${getLatencyClass(server.latency)}`}>{formatLatency(server.latency, language)}</span></div><div className="detail-metric-grid">{detailMetrics.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></section>
...
/* Legacy NAS detail implementation retained as a migration reference. */
/*
...
/* Legacy active OpenWrt detail implementation retained as a migration reference. */
/*
function OpenWrtRouterView({ server, text, language, onBack, onOpen, onConnect, onScan, isScanning, onDiscover, isDiscovering, onEdit, onManager, onCron, onAddCustomService, onDeleteCustomService }: ServerDetailProps) {
  ...
  const overviewFooter = buildRouterOverviewFooter(server, profile, zhMode);
  return <section className="server-detail-view router-detail-view">
    <ServerDetailHeader server={server} text={text} language={language} eyebrow={zhMode ? "OpenWrt 路由器" : "OpenWrt router"} onBack={onBack} router />
