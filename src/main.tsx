...
type Server = { id: string; name: string; host: string; port: number; username: string; system: string; status: ServerStatus; profile?: ServerProfile };
...
function App() {
  ...
  const requestFor = () => ({ host: form.host.trim(), port: Number(form.port), username: form.username.trim(), authMethod: form.authMethod, password: form.authMethod === "password" ? form.password : null, privateKeyPath: form.authMethod === "privateKey" ? form.privateKeyPath.trim() : null, passphrase: form.passphrase || null });
  ...
  const connect = async () => {
    ...
    try {
      const result = await invoke<{ system: string }>("test_ssh_connection", { request: requestFor() });
      ...
      persistServers(next); setServer(nextServer); setWizardOpen(false);
    } catch (connectionError) { setError(typeof connectionError === "string" ? connectionError : "连接失败，请检查地址、端口和登录方式。"); }
  ...
  const scanServer = async () => {
    ...
    try {
      const profile = await invoke<ServerProfile>("inspect_server", { request: requestFor() });
      const updated = { ...server, profile };
      setServer(updated); persistServers([updated, ...servers.filter((item) => item.id !== updated.id)]);
    } catch (scanError) { setError(typeof scanError === "string" ? scanError : "扫描失败，请重新连接服务器后再试。"); }
    finally { setScanning(false); }
  };

  const selectServer = (selected: Server) => { setServer(selected); setError(""); };

  return <main className="shell">
    <aside className="sidebar">
      <div className="brand"><img className="brand-icon" src="/opsnest-icon.png" alt="" /><span>OpsNest</span></div>
      <nav aria-label="主导航"><a className="active" href="#hosts">我的服务器</a><a href="#tasks">任务记录</a><a href="#settings">设置</a></nav>
    ...
    <section className="content">
      ...
      {server ? <section className="server-view" id="hosts">
        <div className="server-card"><div className="server-card-top"><div className="server-orb">⌁</div><span className={`connected-badge ${server.status === "saved" ? "saved-badge" : ""}`}>● {server.status === "connected" ? "已连接" : "已保存"}</span></div><h2>{server.name}</h2><p className="server-address">{server.username}@{server.host}:{server.port}</p><div className="server-meta"><div><span>系统</span><strong>{server.profile?.osName ?? server.system}</strong></div><div><span>连接方式</span><strong>SSH</strong></div></div><button className="primary" onClick={openWizard}>添加另一台服务器</button></div>
        {server.profile ? <div className="profile-panel"><div className="profile-heading"><div><p className="eyebrow">AI 服务器档案</p><h2>我已经了解这台服务器</h2></div><span className="read-only-pill">只读扫描</span></div><p className="profile-summary">已读取基础环境信息。没有修改文件、安装软件或启动服务。</p><div className="profile-grid"><div><span>主机名</span><strong>{server.profile.hostname}</strong></div><div><span>CPU</span><strong>{server.profile.cpuCores} 核</strong></div><div><span>内存</span><strong>{server.profile.memory}</strong></div><div><span>磁盘</span><strong>{server.profile.disk}</strong></div><div><span>Docker</span><strong>{server.profile.dockerInstalled ? `已安装 · ${server.profile.dockerContainers} 个运行中` : "未安装"}</strong></div></div><button className="text-button" onClick={scanServer}>重新扫描</button></div> : <button className="next-step clickable" onClick={scanServer} disabled={isScanning}><span className="step-icon">✦</span><div><strong>{isScanning ? "正在了解这台服务器…" : "下一步：让 AI 了解这台服务器"}</strong><p>{isScanning ? "只读取基础环境信息，请稍候。" : "读取系统、资源和 Docker 状态，不会自动修改任何内容。"}</p></div><span className="arrow">→</span></button>}
      </section> : <section className="empty-state" id="hosts"><div className="hero-icon">⌁</div><h2>连接你的第一台服务器</h2><p>输入 IP 地址、用户名和密码，然后用人话描述你想做什么。</p><button className="primary" onClick={openWizard}>开始连接</button><button className="secondary">查看演示</button></section>}
